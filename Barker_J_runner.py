...
from Barker_J_player import RandomBot, Best_AI_bot
...
turn = white
board = []
possible_moves = {5,11,17,23,29,35,41}
first_turn = 0
...
def whose_turn(my_board, prev_turn):
    ...
    for i in range(len(my_board)):
        for j in range(len(my_board[i])):
            if (prev_turn == black and my_board[i][j] == 'X') or (prev_turn == white and my_board[i][j] == 'O'):
                for incr in directions:
                    x_pos = i + incr[0]
                    y_pos = j + incr[1]
                    count = 1
                    stop = False
                    while 0 <= x_pos < x_max and 0 <= y_pos < y_max and not stop:
...
def find_moves(my_board, my_color):
    ...
    for i in range(len(my_board)):
        ...
        for incr in [[0, -1]]:
            y_pos = j + incr[1]
            stop = False
            while 0 <= y_pos < y_max and not stop:
                if my_board[i][y_pos] == '.':
                    moves_found.add(i * y_max + y_pos)
                    stop = True
                y_pos += incr[1]
    return moves_found
